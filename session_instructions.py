...
class SessionInstructionsHandler(webapp2.RequestHandler):
    def get(self):
        session_treatment = self.request.get('session_treatment', default_value="rc")
        group_id = self.request.get('group_id', default_value="TEST")
        instructions = self.get_instructions(session_treatment, group_id)
        ...
        self.response.write(jinja_render('Session_Instructions.html', template_values))

    def get_instructions(self, session_treatment, group_id):
        if self.request.get('tour_complete', default_value=0) == "1":
